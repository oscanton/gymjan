...
*   **Múltiples Menús:** Soporte para cambiar entre diferentes archivos de datos de menús (ej. `menus/menu.js`, `menus/menu_1.js`).
...
*   **Rutinas Diarias:** Asignación de rutinas por día de la semana.
*   **Estimación de Gasto:** Calcula el gasto calórico extra basado en la rutina seleccionada.
*   **Catálogo de Rutinas:** Muestra descripciones y ejercicios asociados a la rutina elegida.
...
├── js/
│   ├── core/
│   │   ├── config.js     # Configuración global y lista de menús
│   │   └── formulas.js   # Lógica matemática (Harris-Benedict, Macros, etc.)
│   ├── data/
│   │   ├── menus/        # Datos de menús
│   │   ├── rutinas/      # Datos de rutinas
│   │   ├── ejercicios.js # Catálogo de ejercicios
│   │   └── foods.js      # Base de datos de alimentos (nutrición por 100g)
...
│       ├── actividad.page.js
│       └── control.page.js
└── index.html            # Punto de entrada (implícito)
```
...
2.  **Ejecutar:**
    ...
    *Recomendación:* Utiliza una extensión como **Live Server** en VS Code para evitar problemas con las políticas de CORS al cargar módulos JS dinámicamente.

## ⚙️ Configuración

### Añadir un nuevo menú
1.  Crea un archivo nuevo en `js/data/menus/` (ej. `menu_verano.js`).
2.  Define la estructura `window.MENU_DATA = [...]` siguiendo el formato de `menus/menu.js`.
3.  Registra el archivo en `js/core/config.js`:
    ```javascript
    const AVAILABLE_MENUS = [
        { label: 'Menú Base', file: 'menus/menu.js' },
        { label: 'Menú Verano', file: 'menus/menu_verano.js' },
    ];
    ```
